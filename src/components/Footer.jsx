  ...
  return (
    <footer id="footer" className="flex justify-center items-center h-screen relative bg-cover bg-center" style={{ backgroundImage: `url(${lord})` }}>
      ...
      <div className="flex z-30 relative flex-col gap-5 text-white w-full animate-fade-down">
        <h1 className="text-5xl mb-3 lg:text-5xl self-center font-bold text-transparent bg-clip-text bg-gradient-to-r from-cyan-200 via-blue-300 to-indigo-400">
          Contact Me
        </h1>

        {/* Contact Form */}
        <form ref={form} onSubmit={sendEmail} className="flex mb-10 flex-col self-center md:w-130 md:px-8 lg:w-200 lg:rounded-lg lg:p-10 h-120 w-100 gap-3 backdrop-blur-md bg-white/20 p-5 lg:px-15 shadow-lg">
          ...
          <button type="submit" className="bg-gradient-to-r hover:cusor-pointer text-lg font-semibold from-cyan-400 via-blue-300 to-indigo-500 text-white py-2 mt-2 px-4 w-40 self-center rounded transition">
            Send
          </button>
        </form>
      </div>
      ...
      <div className="absolute bottom-0 py-6 w-full border-t-1 border-white/10 backdrop-blur-lg lg:bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 flex flex-col lg:flex-row-reverse items-center justify-around gap-3">
        ...
        <span className="text-white text-md text-center">
          &#169; Copyright Mengkhey Khorn. Phone Number +011359292
        </span>
      </div>
    </footer>
  );
